...
public class InformationRepositoryImpl implements InformationRepository {
    ...
    public void delete(String informationId) {
        if (informationId == null || informationId.trim().isEmpty()) {
            throw new IllegalArgumentException("Information ID cannot be null or empty");
        }
        
        String sql = """
                DELETE FROM "Cohorte".information
                WHERE id = ?
                """;

        try (PreparedStatement stmt = getConnection().prepareStatement(sql)) {
            try {
                UUID uuid = UUID.fromString(informationId.trim());
                stmt.setObject(1, uuid);
            } catch (IllegalArgumentException e) {
                // If it's not a valid UUID, try to find by string ID
                stmt.setString(1, informationId.trim());
            }
            
            int deletedRows = stmt.executeUpdate();
            if (deletedRows == 0) {
                throw new SQLException("Information with ID '" + informationId + "' was not found or could not be deleted.");
            }
        } catch (SQLException e) {
            throw new RuntimeException("Failed to delete information: " + e.getMessage(), e);
        }
    }
}
